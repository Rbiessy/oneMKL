...
void rand_matrix(std::vector<fpType> &m, oneapi::mkl::layout layout_val, std::size_t nrows,
    ...
    const bool swap_ld = (layout_val == oneapi::mkl::layout::col_major) != (transpose_val != oneapi::mkl::transpose::nontrans);
    auto [outer_size, inner_size] = swap_if_cond(swap_ld, nrows, ncols);
    if (inner_size > ld) {
...
inline bool require_coo_sorted_by_row(sycl::queue queue) {
    ...
    return vendor_id == oneapi::mkl::device::nvidiagpu || vendor_id == oneapi::mkl::device::amdgpu;
}
...
template <typename fpType, typename intType>
void shuffle_sparse_matrix(sycl::queue queue, sparse_matrix_format_t format, intType indexing, intType *ia,
    ...
    if (format == sparse_matrix_format_t::CSR) {
        ...
        if (require_coo_sorted_by_row(queue)) {
            ...
            for (std::size_t i = 0; i < nrows; ++i) {
                ...
                while (linear_idx + nnz_row < static_cast<std::size_t>(nnz) && ia[linear_idx] == ia[linear_idx + nnz_row]) {
                    ++nnz_row;
                }
