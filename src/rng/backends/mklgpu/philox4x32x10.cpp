...
public:
    philox4x32x10_impl(sycl::queue queue, std::uint64_t seed)
            : oneapi::math::rng::detail::engine_impl(queue), engine_((oneapi::mkl::rng::philox4x32x10(queue, seed))) {
    }
    ...
    philox4x32x10_impl(const philox4x32x10_impl* other)
            : oneapi::math::rng::detail::engine_impl(*other), engine_((oneapi::mkl::rng::philox4x32x10(other->engine_))) {
    }
    ...
    virtual void generate(const bits<std::uint32_t>& /*distr*/, std::int64_t /*n*/,
        ...
        throw unimplemented("rng/mklgpu", "philox4x32x10::generate", "bits distribution is not supported");
    }
